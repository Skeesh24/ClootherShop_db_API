...
def item_by_id(item_id: int, db: Session = Depends(get_db)):
    ...
    res = select_where(items, db, items.c['id'] == item_id).first()
    # response = ItemResponse(**dict(res)).__dict__
...
def insert_item(item: ItemRequest, db: Session = Depends(get_db)):
    try:
        ...
        index = len(select_all(items, db))+1
...
def update_item(item_id: int, item: ItemRequest, db: Session = Depends(get_db)):
    old_item = select_where(items, db, items.c['id'] == item_id).first()
    if old_item is None:
